...
public class TreeTraverser {
  ...
  public static List<TreeNode<?>> breadthTraversal(TreeNode<?> root) {
    Queue<TreeNode<?>> queue = new ArrayDeque<>();
    List<TreeNode<?>> beadthOrder = new ArrayList<>();
    if (root == null) {
      return beadthOrder;
    }
  ...
  public static List<TreeNode<?>> preorderIterative(TreeNode<?> root) {
    ...
    while (CollectionUtils.isNotEmpty(stack)) {
      ...
      Lists.reverse(current.getChildren()).forEach(stack::push);
    }
    return preorder;
  }
}
